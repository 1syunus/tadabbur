...
import { normalizeNoteSection } from '@/lib/api/sections/normalized'
...
export async function GET(_: NextRequest, context: RouteContext) {
  try {
    ...
    const rawSection = await sectionsService.getSectionById(id)
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  try {
    ...
    const sectionsService = new SectionsService(supabase)
    const rawSection = await sectionsService.updateSection(id, bodyValidation.data)
